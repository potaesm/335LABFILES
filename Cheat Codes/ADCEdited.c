...
void InitADC1(void) {
	...
	/* Step x. compare setup */
	ADC->ADCMPR[0].CMPD = 0x250;	// Comparison Data 0x250
	ADC->ADCMPR[0].CMPCH = 1;			// Compare Channel Selection
	ADC->ADCMPR[0].CMPCOND = 0;		// Compare Condition
	// 1: greater or equal, 0: less than
	ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
	ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
void InitADC2(void) {
	/* Step 1. GPIO initial */ 
	GPIOA->OFFD |= 0x00040000;		// Disable digital input path (when input is analog signal)
	...
	/* Step 4. Select ADC channel */
	ADC->ADCHER.CHEN = 0x4;		// 8-bits -> ch2
	...
	/* Step x. compare setup */
	ADC->ADCMPR[0].CMPD = 0x250;	// Comparison Data 0x250
	ADC->ADCMPR[0].CMPCH = 2;			// Compare Channel Selection
	ADC->ADCMPR[0].CMPCOND = 0;		// Compare Condition
	// 1: greater or equal, 0: less than
	ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
	ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
void InitADC3(void) {
	...
	/* Step 4. Select ADC channel */
	ADC->ADCHER.CHEN = 0x8;		// 8-bits -> ch3
	...
	/* Step x. compare setup */
	ADC->ADCMPR[0].CMPD = 0x250;	// Comparison Data 0x250
	ADC->ADCMPR[0].CMPCH = 3;			// Compare Channel Selection
	ADC->ADCMPR[0].CMPCOND = 0;		// Compare Condition
	// 1: greater or equal, 0: less than
	ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
	ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
void InitADC4(void) {
	/* Step 1. GPIO initial */ 
	GPIOA->OFFD |= 0x00100000;		// Disable digital input path (when input is analog signal)
	...
	/* Step 4. Select ADC channel */
	ADC->ADCHER.CHEN = 0x10;		// 8-bits -> ch4
...
void InitADC5(void) {
	/* Step 1. GPIO initial */ 
	GPIOA->OFFD |= 0x00200000;		// Disable digital input path (when input is analog signal)
	SYS->GPAMFP.ADC5_AD8 = 1;					// Set ADC function 
	...
	/* Step 4. Select ADC channel */
	ADC->ADCHER.CHEN = 0x20;		// 8-bits -> ch5
	...
	/* Step x. compare setup */
	ADC->ADCMPR[0].CMPD = 0x250;	// Comparison Data 0x250
	ADC->ADCMPR[0].CMPCH = 5;			// Compare Channel Selection
	ADC->ADCMPR[0].CMPCOND = 0;		// Compare Condition
	// 1: greater or equal, 0: less than
	ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
	ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
void InitADC6(void) {
	/* Step 1. GPIO initial */ 
	GPIOA->OFFD |= 0x00400000;		// Disable digital input path (when input is analog signal)
	...
	/* Step 4. Select ADC channel */
	ADC->ADCHER.CHEN = 0x40;		// 8-bits -> ch6
	...
	/* Step x. compare setup */
	ADC->ADCMPR[0].CMPD = 0x250;	// Comparison Data 0x250
	ADC->ADCMPR[0].CMPCH = 6;			// Compare Channel Selection
	ADC->ADCMPR[0].CMPCOND = 0;		// Compare Condition
	// 1: greater or equal, 0: less than
	ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
	ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
void InitADC7(void) {
	/* Step 1. GPIO initial */ 
	GPIOA->OFFD |= 0x00800000;		// Disable digital input path (when input is analog signal)
...
//------------------------------------------------------------------------------------------Timer1 IRQ for Write I2C Data to SDA(GPA10), SCL(GPA11)
...
//------------------------------------------------------------------------------------------Timer0 IRQ using ADC0 GPA0
/*void TMR0_IRQHandler(void) {	// Timer0 interrupt subroutine 
	char adc_value[15] = "ADC0 Value:";
	char adc_value2[15] = "A0 0xABC ZZZZd";
	while (ADC->ADSR.ADF == 0);	// A/D Conversion End Flag
	// A status flag that indicates the end of A/D conversion.
		
	ADC->ADSR.ADF = 1;					// This flag can be cleared by writing 1 to self
	sprintf(adc_value+11,"%d   ",ADC->ADDR[0].RSLT);
	print_lcd(2, adc_value);
	sprintf(adc_value2+5, "%X %dd    ", ADC->ADDR[0].RSLT, ADC->ADDR[0].RSLT);
	print_lcd(3, adc_value2);
	ADC->ADCR.ADST = 1;					// 1 = Conversion start
	
	if (ADC->ADDR[0].RSLT > 0x250) {
			GPC_12 = 1;
			GPC_13 = 1;
			ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
			ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
//------------------------------------------------------------------------------------------Timer0 IRQ using ADC1 GPA1
/*void TMR0_IRQHandler(void) {	// Timer0 interrupt subroutine 
	char adc_value[15] = "ADC1 Value:";
	char adc_value2[15] = "A1 0xABC ZZZZd";
	while (ADC->ADSR.ADF == 0);	// A/D Conversion End Flag
	// A status flag that indicates the end of A/D conversion.
		
	ADC->ADSR.ADF = 1;					// This flag can be cleared by writing 1 to self
	sprintf(adc_value+11,"%d   ",ADC->ADDR[1].RSLT);
	print_lcd(2, adc_value);
	sprintf(adc_value2+5, "%X %dd    ", ADC->ADDR[1].RSLT, ADC->ADDR[1].RSLT);
	print_lcd(3, adc_value2);
	ADC->ADCR.ADST = 1;					// 1 = Conversion start
	
	if (ADC->ADDR[1].RSLT > 0x250) {
			GPC_12 = 1;
			GPC_13 = 1;
			ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
			ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
//------------------------------------------------------------------------------------------Timer0 IRQ using ADC2 GPA2
/*void TMR0_IRQHandler(void) {	// Timer0 interrupt subroutine 
	char adc_value[15] = "ADC2 Value:";
	char adc_value2[15] = "A2 0xABC ZZZZd";
	while (ADC->ADSR.ADF == 0);	// A/D Conversion End Flag
	// A status flag that indicates the end of A/D conversion.
		
	ADC->ADSR.ADF = 1;					// This flag can be cleared by writing 1 to self
	sprintf(adc_value+11,"%d   ",ADC->ADDR[2].RSLT);
	print_lcd(2, adc_value);
	sprintf(adc_value2+5, "%X %dd    ", ADC->ADDR[2].RSLT, ADC->ADDR[2].RSLT);
	print_lcd(3, adc_value2);
	ADC->ADCR.ADST = 1;					// 1 = Conversion start
	
	if (ADC->ADDR[2].RSLT > 0x250) {
			GPC_12 = 1;
			GPC_13 = 1;
			ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
			ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
//------------------------------------------------------------------------------------------Timer0 IRQ using ADC3 GPA3
/*void TMR0_IRQHandler(void) {	// Timer0 interrupt subroutine 
	char adc_value[15] = "ADC3 Value:";
	char adc_value2[15] = "A3 0xABC ZZZZd";
	while (ADC->ADSR.ADF == 0);	// A/D Conversion End Flag
	// A status flag that indicates the end of A/D conversion.
	...
	print_lcd(2, adc_value);
	sprintf(adc_value2+5, "%X %dd    ", ADC->ADDR[3].RSLT, ADC->ADDR[3].RSLT);
	print_lcd(3, adc_value2);
	ADC->ADCR.ADST = 1;					// 1 = Conversion start
	
	if (ADC->ADDR[3].RSLT > 0x250) {
			GPC_12 = 1;
			GPC_13 = 1;
			ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
			ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
//------------------------------------------------------------------------------------------Timer0 IRQ using ADC5 GPA5
/*void TMR0_IRQHandler(void) {	// Timer0 interrupt subroutine 
	char adc_value[15] = "ADC5 Value:";
	char adc_value2[15] = "A5 0xABC ZZZZd";
	while (ADC->ADSR.ADF == 0);	// A/D Conversion End Flag
	// A status flag that indicates the end of A/D conversion.
		
	ADC->ADSR.ADF = 1;					// This flag can be cleared by writing 1 to self
	sprintf(adc_value+11,"%d   ",ADC->ADDR[5].RSLT);
	print_lcd(2, adc_value);
	sprintf(adc_value2+5, "%X %dd    ", ADC->ADDR[5].RSLT, ADC->ADDR[5].RSLT);
	print_lcd(3, adc_value2);
	ADC->ADCR.ADST = 1;					// 1 = Conversion start
	
	if (ADC->ADDR[5].RSLT > 0x250) {
			GPC_12 = 1;
			GPC_13 = 1;
			ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
			ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
//------------------------------------------------------------------------------------------Timer0 IRQ using ADC6 GPA6
void TMR0_IRQHandler(void) {	// Timer0 interrupt subroutine 
	char adc_value[15] = "ADC6 Value:";
	char adc_value2[15] = "A6 0xABC ZZZZd";
	while (ADC->ADSR.ADF == 0);	// A/D Conversion End Flag
	// A status flag that indicates the end of A/D conversion.
		
	ADC->ADSR.ADF = 1;					// This flag can be cleared by writing 1 to self
	sprintf(adc_value+11,"%d   ",ADC->ADDR[6].RSLT);
	print_lcd(2, adc_value);
	sprintf(adc_value2+5, "%X %dd    ", ADC->ADDR[6].RSLT, ADC->ADDR[6].RSLT);
	print_lcd(3, adc_value2);
	ADC->ADCR.ADST = 1;					// 1 = Conversion start
	
	if (ADC->ADDR[6].RSLT > 0x250) {
			GPC_12 = 1;
			GPC_13 = 1;
			ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
			ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
...
//------------------------------------------------------------------------------------------Timer0 IRQ using ADC7 GPA7
/*void TMR0_IRQHandler(void) {	// Timer0 interrupt subroutine 
	char adc_value[15] = "ADC7 Value:";
	char adc_value2[15] = "A7 0xABC ZZZZd";
	while (ADC->ADSR.ADF == 0);	// A/D Conversion End Flag
	// A status flag that indicates the end of A/D conversion.
	...
	print_lcd(2, adc_value);
	sprintf(adc_value2+5, "%X %dd    ", ADC->ADDR[7].RSLT, ADC->ADDR[7].RSLT);
	print_lcd(3, adc_value2);
	ADC->ADCR.ADST = 1;					// 1 = Conversion start
	
	if (ADC->ADDR[7].RSLT > 0x250) {
			GPC_12 = 1;
			GPC_13 = 1;
			ADC->ADCMPR[0].CMPIE = 1;			// Compare Interrupt Enable
			ADC->ADCMPR[0].CMPEN = 1;			// Compare Enable
